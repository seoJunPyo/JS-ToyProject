;(function () {
  ...
  const baseball = {
    ...
    $quesetion: get('.ball-question'),
    $answer: get('.ball-answer'),
    $input: get('.ball-input'),
  }

  const { limit, digit, $quesetion, $answer, $input } = baseball
  ...
  const onPlayed = (number, hint) => {
    return `<em>${trial}</em>차 시도 : ${number}, ${hint}`
  }
  ...
  const getbResult = (number, answer) => {
    ...
    const balls = getbBalls(number, answer)
  ...
  const playGame = (e) => {
    ...
    if (inputNumber.length !== digit) {
      alert(`${digit}자리 숫자입력하세요`)
    } else if (isDupicate(inputNumber)) {
      alert('중복 숫자가 있습니다.')
    } else {
      trial++
      const result = onPlayed(inputNumber, getbResult(inputNumber, password))
      $quesetion.innerHTML += `<span>${result}</span>`
